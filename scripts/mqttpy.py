        ...
        while True:
            ...
            try:
                ...
                if messageID == 0x101:
                    ...
                    if getTimeDelayed(timestamp_3B6, 1): #only update odometer every second
                        Odometer(client, msg)
                        timestamp_3B6 = time.time()
                else:
...
def ESP_status(client, msg):
    ...
    localDict = {'Not_Applied': 'Not Applied', 'Driver_applying_brakes': 'Applying'}
    decoded = db.decode_message(msg.arbitration_id, msg.data)
    #print(decoded)

    decodedBrakeStatus = translateResponse(localDict, decoded['ESP_driverBrakeApply'])
